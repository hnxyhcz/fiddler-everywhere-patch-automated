...
(async () => {
  ...
  // 语言设置页面交互 - 动态添加语言选择
  const initLanguageSettings = () => {
    const appearanceSettings = document.querySelector('.appearance-settings-page-content');
    if (!appearanceSettings) return;

    // 检查是否已添加语言设置
    if (appearanceSettings.querySelector('.language-settings-block')) return;

    // 创建语言设置HTML
    const languageHTML = `
      <input name="language" class="hidden ng-untouched ng-pristine ng-valid">
      <div class="language-settings-block"><strong>Language</strong>
        <div class="themes language-themes">
          <div class="theme-block language-block k-checked" data-lang="zhCn">
            <fdl-icon name="theme" variant="extra" data-icon="theme" class="icon">
              <svg preserveAspectRatio="xMidYMid" class="fdl-icon fdl-element ng-star-inserted" style="aspect-ratio: 154 / 106;">
                <use href="#icon-extra-theme"></use>
              </svg>
            </fdl-icon>
            <span>简体中文</span>
          </div>
          <div class="theme-block language-block" data-lang="en">
            <fdl-icon name="theme" variant="extra" data-icon="theme" class="icon">
              <svg preserveAspectRatio="xMidYMid" class="fdl-icon fdl-element ng-star-inserted" style="aspect-ratio: 154 / 106;">
                <use href="#icon-extra-theme"></use>
              </svg>
    ...
    // 在主题设置之前插入
    const themeInput = appearanceSettings.querySelector('input[name="themeName"]');
    if (themeInput) {
      themeInput.insertAdjacentHTML('beforebegin', languageHTML);
    } else {
      // 如果找不到themeName input，直接插入到开头
      appearanceSettings.insertAdjacentHTML('afterbegin', languageHTML);
    }

    // 设置当前语言的选中状态并添加点击事件
    const languageBlocks = appearanceSettings.querySelectorAll('.language-block');
    languageBlocks.forEach(block => {
      if (block.dataset.lang === lang) {
        block.classList.add('k-checked');
      } else {
        block.classList.remove('k-checked');
      }

      // 添加点击事件
      block.addEventListener('click', () => {
        const newLang = block.dataset.lang;
        if (newLang && newLang !== lang) {
          switchLanguage(newLang);
          // 更新选中状态
          languageBlocks.forEach(b => b.classList.remove('k-checked'));
          block.classList.add('k-checked');
          console.info('语言切换成功:', newLang);
        }
      });
    });
  };
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // 监听外观设置面板变化
    let windowObserverCreated = false;
    const settingsObserver = new MutationObserver(() => {
      initLanguageSettings();
      // 监听 kendo-window 变化（只创建一次）
      if (!windowObserverCreated) {
        const kendoWindow = document.querySelector('kendo-window');
        if (kendoWindow) {
          windowObserverCreated = true;
          const windowObserver = new MutationObserver(updateAppearanceSettingsHeight);
          windowObserver.observe(kendoWindow, {
            childList: true,
            subtree: true,
            attributes: true,
          });
      ...
      updateAppearanceSettingsHeight();
    });
    settingsObserver.observe(document.body, { childList: true, subtree: true });
    // 初始化语言设置
    initLanguageSettings();
    // 监听窗口大小变化
    window.addEventListener('resize', updateAppearanceSettingsHeight);
  });

  // 计算并更新外观设置面板高度
  const updateAppearanceSettingsHeight = () => {
    const kendoWindow = document.querySelector('kendo-window');
    const appearanceSettings = document.querySelector('app-appearance-settings');
    if (!kendoWindow || !appearanceSettings) return;

    const parentElement = appearanceSettings.parentElement;
    if (parentElement) {
      parentElement.style.height = `${kendoWindow.offsetHeight - 96}px`;
    }
  };
})();
